...
module CubicNumber = struct
  ...
  let extract_omega (q : Q_rootp.t) =
    ...
    let u = Q_rootp.add q (Q_rootp.neg (Q_rootp.mul v (Q_rootp.make_root (-3)))) in
  ...
  let of_q_rootp q =
    ...
    if Q_rootp.is_zero q then zero
      ...
      if not (Q_rootp.is_zero omega_p) then (
        let b_omega = { CubicBasis.roots = IdMap.empty; CubicBasis.omega = 1 } in
      ...
      IdMap.merge
        (fun id e1 e2 ->
          ...
          if sum >= 3 then (
            ...
            for _ = 1 to (sum / 3) do res_coeff := Q_rootp.mul !res_coeff w done;
  ...
  let normalize x =
    ...
    let add_term b c =
      if Q_rootp.is_zero c then ()
        ...
        let old = CubicBasisMap.find_opt b !temp_map |> Option.value ~default:Q_rootp.zero in
        temp_map := CubicBasisMap.add b (Q_rootp.add old c) !temp_map
    in
    (* Step 1: 立方根部分の簡約 *)
    x.terms |> CubicBasisMap.iter (fun b c ->
      let current_coeff = ref c in
      let new_roots = ref IdMap.empty in
      b.roots |> IdMap.iter (fun id p ->
        let w = global_l2_registry.id_to_w.(id) in
    ...
    let add_final b c =
      if Q_rootp.is_zero c then ()
        ...
        let old = CubicBasisMap.find_opt b !final_map |> Option.value ~default:Q_rootp.zero in
        final_map := CubicBasisMap.add b (Q_rootp.add old c) !final_map
    in
    !temp_map |> CubicBasisMap.iter (fun b c ->
      let rem = ((b.omega mod 3) + 3) mod 3 in
      if rem = 2 then (
        add_final { b with omega = 0 } (Q_rootp.neg c);
        add_final { b with omega = 1 } (Q_rootp.neg c)
      ) else (
        add_final { b with omega = rem } c
      )
    );
    let cleaned_map = CubicBasisMap.filter (fun _ c -> not (Q_rootp.is_zero c)) !final_map in
    { terms = cleaned_map }
  ...
  let mul x y =
    let res = ref CubicBasisMap.empty in
    CubicBasisMap.iter (fun b1 c1 ->
      CubicBasisMap.iter (fun b2 c2 ->
        let coeff_l1, b_prod = multiply_basis b1 b2 in
        let new_val = Q_rootp.mul (Q_rootp.mul c1 c2) coeff_l1 in
        let old_val = CubicBasisMap.find_opt b_prod !res |> Option.value ~default:Q_rootp.zero in
  ...
  let split_omega x =
    ...
    CubicBasisMap.iter (fun b c ->
      ...
      else if b.omega = 0 then
        a_terms := CubicBasisMap.add b c !a_terms
    ...
    if is_zero b then (
      ...
      let rec gen_combinations = function
        ...
        | id :: rest ->
            ...
            let with1 = List.map (fun (b : CubicBasis.t) -> { b with roots = IdMap.add id 1 b.roots }) sub in
            let with2 = List.map (fun (b : CubicBasis.t) -> { b with roots = IdMap.add id 2 b.roots }) sub in
            sub @ with1 @ with2
      in
      ...
      let ab = mul a b in
      let bb = mul b b in
      let norm = add (add aa (neg ab)) bb in (* Norm = A^2 - AB + B^2 *)
      let inv_norm = inv norm in (* 再帰: normはomegaを含まないはずなので上の分岐に入る *)
      let num_real = add a (neg b) in (* Numerator Real part: A - B *)
      let num_imag = neg b in         (* Numerator Omega part: -B *)
      ...
      let res_imag_shifted = 
        ...
        CubicBasisMap.iter (fun b c ->
          m := CubicBasisMap.add { b with omega = 1 } c !m
        ) res_imag.terms;
        { terms = !m }
      in
  ...
  let register_as_basis w =
    ...
    let id =
      ...
      Array.iteri (fun i w_reg -> if Q_rootp.equal w w_reg then existing_id := i) global_l2_registry.id_to_w;
      if !existing_id <> -1 then !existing_id
      else (
        let new_id = Array.length global_l2_registry.id_to_w in
        global_l2_registry.id_to_w <- Array.append global_l2_registry.id_to_w [| w |];
        new_id
      )
    in
  ...
  let basis_to_latex (b : CubicBasis.t) =
    let o_str = match b.omega with 0 -> "" | 1 -> "\\omega" | 2 -> "\\omega^2" | _ -> "" in
    let r_str =
      if IdMap.is_empty b.roots then ""
      else
        let terms = IdMap.fold (fun id p acc ->
          let w_str = Q_rootp.to_string_latex global_l2_registry.id_to_w.(id) in
